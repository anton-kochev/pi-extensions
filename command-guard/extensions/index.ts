...
const CONFIG_FILE = join(".pi", "command-guard.json");
const DEFAULT_COMMAND_RULES: RawRule[] = [];
const DEFAULT_PATH_RULES: RawRule[] = [
	{
		name: "protect command-guard config",
		pattern: "^\\.pi/command-guard\\.json$",
		action: "confirm",
	},
];

export default function commandGuard(pi: ExtensionAPI) {
	let state: CommandGuardState | undefined;
	let enabled = true;
	...
	pi.registerCommand("command-guard", {
		description: "View, reload, and toggle command-guard command/path rules",
		handler: async (args, ctx) => {
			...
			if (command === "help") {
				return emitText(ctx, COMMAND_GUARD_HELP);
			}
			...
			if (command === "reload") {
				state = loadState(ctx.cwd);
				return emitText(ctx, `Reloaded command-guard rules.\n\n${formatStatus(state, enabled)}`, state.warnings.length > 0 ? "warning" : "info");
			}

			if (command === "toggle") {
				enabled = !enabled;
				persistEnabled(pi, enabled);
				return emitText(ctx, `command-guard is now ${enabled ? "enabled" : "disabled"}.`, enabled ? "info" : "warning");
			}

			return emitText(ctx, "Usage: /command-guard [status|list|reload|toggle|help]", "warning");
		},
	});
}

const COMMAND_GUARD_HELP = `Usage: /command-guard [status|list|reload|toggle|help]

command-guard protects agent-run bash commands and agent file edits/writes using project rules from .pi/command-guard.json.
It does not guard user-entered ! or !! shell commands.

Create .pi/command-guard.json in your project, for example:
...
For backward compatibility, command rules may also be placed in a top-level "rules" array.
...
Commands:
- /command-guard status  Show config path and rule count.
- /command-guard list    Show effective rules.
- /command-guard reload  Reload .pi/command-guard.json from disk.
- /command-guard toggle  Enable/disable command-guard for this session.
- /command-guard help    Show this help.`;

async function guardValue(
	ctx: ExtensionContext,
	state: CommandGuardState,
	kind: RuleKind,
	value: string,
	label: string,
	...
	if (blockingRule) {
		return {
			block: true,
			reason: `Blocked by command-guard ${kind} rule "${blockingRule.name}"`,
		};
	}
	...
	if (!ctx.hasUI) {
		return {
			block: true,
			reason: `${capitalize(kind)} requires confirmation by command-guard rule "${ruleNames}", but no UI is available`,
		};
	}
	...
	if (choice !== "Yes") {
		return {
			block: true,
			reason: "Blocked by command-guard: user declined confirmation",
		};
	}
...
function loadState(cwd: string): CommandGuardState {
...
function formatStatus(state: CommandGuardState, enabled = true): string {
	...
	const lines = [
		`command-guard is ${enabled ? "enabled" : "disabled"}.`,
...
function formatRuleList(state: CommandGuardState, enabled = true): string {
...
function formatWarnings(state: CommandGuardState): string {
	return ["command-guard warnings:", ...state.warnings.map((warning) => `- ${warning}`)].join("\n");
}
...
function restoreEnabled(ctx: ExtensionContext): boolean {
	for (const entry of [...ctx.sessionManager.getEntries()].reverse()) {
		if (entry.type !== "custom" || entry.customType !== "command-guard-enabled") continue;
...
function persistEnabled(pi: ExtensionAPI, enabled: boolean): void {
	pi.appendEntry("command-guard-enabled", { enabled });
}
