...
describe("context-bar package metadata", () => {
  ...
  it("publishes the standalone Pi extension under the expected identity", () => {
    ...
    assert.equal(packageJson.name, "@anton-kochev/context-bar");
    assert.equal(packageJson.repository.directory, "context-bar");
  ...
  it("is wired into local development, repository docs, and trusted publishing", () => {
    ...
    const workflow = readFileSync(
      resolve(root, ".github/workflows/publish-context-bar.yml"),
      "utf8",
    );

    assert.match(readme, /@anton-kochev\/context-bar/);
    assert.match(workflow, /tags: \["context-bar-v\*"\]/);
    assert.match(workflow, /working-directory: context-bar/);
