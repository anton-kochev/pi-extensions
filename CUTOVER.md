...
3. keeps exact `npm:<version>` pins under `pi.extensions`, including separate `@pithos-kit/plan` and `@pithos-kit/skills` entries;
4. upgrades Skills to a release that no longer bundles Plan mode before enabling `@pithos-kit/plan`, avoiding duplicate `/plan` handlers;
...
Only after all eleven organization packages and the Pithos update are verified, deprecate every published version of the previous identities with explicit replacement messages:
